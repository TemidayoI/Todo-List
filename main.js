...
const filterTodos = (filter) => {
  ...
  switch (filter) {
    ...
    case 'personal':
      filteredTodos = getPersonalTodos(today);
      loadTodos(filter, filteredTodos);
      break;

    case 'business':
    filteredTodos = getBusinessTodos(today);
    loadTodos(filter, filteredTodos);
    break;
...
const getPersonalTodos = (today) => {
  const personalTodos = todoListArray.filter(
    (todo) =>
...
const getBusinessTodos = (today) => {
    const businessTodos = todoListArray.filter(
      (todo) =>
        dateStringToDate(formatDate(todo.date)) > today &&
        todo.state === 'pending'
    );
